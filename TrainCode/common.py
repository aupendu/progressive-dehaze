...
class Image_Quality_Metric():
	...
	def ssim(self, imgS, imgG, rgb_range=255):
		imgS = imgS/rgb_range
		imgG = imgG/rgb_range
		return self.calssim(imgS.clamp(0, 1), imgG.clamp(0, 1))

	def psnr(self, imgS, imgG, rgb_range=255):
		if rgb_range==255:
			imgS = torch.round(torch.clamp(imgS, 0, 255))
			diff = (imgS-imgG)/255.0
		if rgb_range==1:
			diff = imgS-imgG
		mse = diff.pow(2).mean()
